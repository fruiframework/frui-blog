...
*Author's note: this is my first blog post ever and in my second language at that, so please tell me if something could be improved :) Frui is a framework based on Flutter. Because of that, those who are familiar with Flutter should be able to grasp concepts in this post intuitively.*
...
Many of these widgets can be adapted from Flutter, which has a similar structure to Frui. This way we can also figure out what works and doesn't work, so we end up with an even better API in the end.
